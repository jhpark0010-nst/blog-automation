#!/usr/bin/env python3
"""Evaluator (GitHub Actions 진입점).

pending.json 의 unreviewed 항목 → Anthropic API 점수 매김 →
70점+ candidates.json 승격, 70점- pending 에서 제거.
"""
import json
import logging
import os
...
# 기존 Routine 프롬프트 기반. 정책/생활정보 대상.
SYSTEM_PROMPT = """당신은 한국 정부정책·생활정보 블로그 publickorea.org 의 편집자입니다.
한국어 기사를 받아 일반 국민에게 블로그 포스트로 제공할 가치가 있는지 0~100점으로 평가합니다.

## 평가 기준 (총 100점)

1. **독자 실익 (40점)**: 일반 국민이 직접 혜택·제도를 활용할 수 있는가? 지원금/감면/신청/대상 등 실생활 적용 가능성.
2. **검색 수요 (30점)**: 네이버·구글에서 검색될 만한 주제인가? 제도명, 혜택명, 신청 방법 등.
3. **정보 희소성 (20점)**: 다른 블로그가 이미 많이 다루지 않은 내용인가?
4. **시의성 (10점)**: 최근 발표/시행된 제도이거나 마감 임박 등 시급성이 있는가?

## 감점 대상 (적극적으로 탈락시킬 것)

- 정치/사건사고/인물동정 (예: 장관 참석, 간담회, 브리핑, 축사)
- 단순 홍보/행사 소식 (실익 없음)
- 사설/칼럼/해설 (사실 전달 없이 의견만)
- 이미 종료된 제도/행사

## 출력

반드시 **순수 JSON** 으로만 응답하세요. 마크다운 코드블록 금지.
...
def build_eval_input(items: list[dict]) -> str:
    ...
    return (
        f"다음 기사 {len(items)}건을 평가하고 각 guid 의 score(0~100) JSON 을 리턴하세요:\n\n"
...
def main() -> int:
    ...
    logger.info(
        f"pending {len(pending_items)}건 / unreviewed {len(unreviewed)}건 / 평가 대상 {len(to_eval)}건"
    )
